                ...
                for (String raceUrl : raceUrls) {
                    try {
                        ...
                        for (Element row : rows) {
                            ...
                            String horseUrl = horseLink != null ? horseLink.attr("abs:href") : "";
                            ...
                            Horse horse = new Horse(waku, umaban, name, jockeyName, weight, oddsValue);
                            horse.setHorseUrl(horseUrl);

                            HorseDetailInfo detail = horseDetailCache.get(horseUrl);

                            if (detail == null) {
                                Thread.sleep(500);
                                detail = WebScraper.getHorseDetailInfo(horseUrl);
                                horseDetailCache.put(horseUrl, detail);
                            }
                        ...
                        String raceNoText = raceUrl.substring(raceUrl.length() - 2);
                        int raceNum = Integer.parseInt(raceNoText);
            ...
            for (String listUrl : targetListUrls) {
                ...
                for (String raceUrl : raceUrls) {
                    try {
                        ...
                        List<Horse> horseList = new ArrayList<>();
                        Elements rows = doc.select(".hr-table tbody tr").isEmpty()
                                ? doc.select("table tr")
                                : doc.select(".hr-table tbody tr");
